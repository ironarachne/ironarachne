...
function main() {
  ...
  if (args.length < 5) {
    console.error(
      'Usage: npx tsx scripts/render_layout.ts <seed> <width> <height> <density> <styles (comma separated)>',
    );
    console.error(
      'Example: npx tsx scripts/render_layout.ts my-seed 50 30 0.25 rectangle,circle,l-shape,blob',
    );
    process.exit(1);
  }
  ...
  console.log(
    `Total Doors: ${doors.length} (${doors.filter((d) => d.type === 'secret').length} secret, ${doors.filter((d) => d.state === 'locked').length} locked)`,
  );
