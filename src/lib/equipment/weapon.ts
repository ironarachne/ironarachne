...
export function checkForMissingMatches(): string {
  ...
  for (let i = 0; i < allDomains.length; i++) {
    ...
    for (let j = 0; j < allWeaponCategories.length; j++) {
      const weaponDescriptors = Descriptor.getDescriptorsMatchingType(
        descriptors,
        allWeaponCategories[j],
      );
      const domainWeaponDescriptors = Descriptor.getDescriptorsMatchingTag(
        weaponDescriptors,
        allDomains[i],
      );
...
function getAllDescriptors(): Descriptor.Descriptor[] {
  ...
  const descriptors = [
    ...
    new Descriptor.Descriptor("with three tails ending in barbs", whip, [
      "animals",
      "monsters",
      "war",
    ]),
    new Descriptor.Descriptor("with nine tails ending in barbs", whip, [
      "animals",
      "monsters",
      "war",
    ]),
    new Descriptor.Descriptor("with a wickedly serrated blade", bladed, [
      "war",
      "evil",
      "hate",
      "destruction",
    ...
    new Descriptor.Descriptor("carved with an erupting volcano", woodbodied, [
      "fire",
      "destruction",
    ]),
    ...
    new Descriptor.Descriptor("with a blade enshrouded in darkness", bladed, [
      "darkness",
      "evil",
      "fear",
      "hate",
