...
export function generateRandomLock(seed: string, config: LockGeneratorConfig): Lock {
  ...
  const lockType: LockType = config.allowedLockTypes ? rng.item(config.allowedLockTypes) : rng.item(['mechanical', 'magical']);
  ...
  const isLocked = rng.float(0, 1) < (config.chanceOfBeingLocked !== undefined ? config.chanceOfBeingLocked : 0.5) ? true : false;
