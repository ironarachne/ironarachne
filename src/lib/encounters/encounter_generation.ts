...
import { generate as generateCharacter, getDefaultCharacterGenerationConfig, type Character } from "$lib/characters";
import { generate as generateCreature, getDefaultCreatureGenerationConfig } from "$lib/creatures";
import type { Encounter, EncounterGenerationConfig, EncounterGroupTemplate } from "./encounter_types";
...
export function generateEncounter(seed: string, config: EncounterGenerationConfig): Encounter {
  ...
  if (config.forceUniformSpecies && !config.speciesOverride) {
    ...
    for (let i = 0; i < template.groupTemplates.length; i++) {
      ...
      const groupPossibleSpecies = groupTemplate.speciesTagFilter ? applyTagFilter(sentient(), groupTemplate.speciesTagFilter) : sentient();

      if (possibleSpecies.length === 0) {
        possibleSpecies = groupPossibleSpecies;
      } else {
        possibleSpecies = possibleSpecies.filter(species => groupPossibleSpecies.includes(species));
      }
    }
...
export function generateEncounterGroup(seed: string, template: EncounterGroupTemplate): MobGroup {
  ...
  if (template.isSentient) {
    ...
    const possibleArchetypes = applyTagFilter(getFantasyCombatArchetypes(), template.archetypeTagFilter);
    const possibleSpecies = template.speciesTagFilter ? applyTagFilter(sentientSpecies, template.speciesTagFilter) : sentientSpecies;
    ...
    for (let i = 0; i < count; i++) {
      ...
      mob.species = applyMutators(rng.randomString(16), mob.species, template.speciesMutators) as Species;
      mobs.push(mob);
    }
  ...
  const possibleSpecies = template.speciesTagFilter ? applyTagFilter(nonSentientSpecies, template.speciesTagFilter) : nonSentientSpecies;
