  ...
  return [
    new AssetTemplate(
      ...
      [
        ...
        new Upgrade('Comms', 'Can receive and broadcast signals over great distances.'),
        new Upgrade(
          'Connected',
...
function randomAssetOfType(assetTypeName: string, assetClass: number, rng: RNG.RNG) {
  ...
  let assetName = `Class ${assetClass} ${assetTemplate.name}`;
  let chosenAssetType = new AssetType(assetTemplate.name, '');
  ...
  return new Asset(assetName, description, assetClass, chosenAssetType, upgrades as Upgrade[]);
}
