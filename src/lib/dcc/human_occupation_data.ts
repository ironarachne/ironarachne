...
/**
 * The human occupation table from the DCC rulebook.
 *
 * Shared and read-only. Several `apply` handlers rewrite the occupation they are attached to —
 * the farmer row replaces its own `name` with the crop it rolled — so a caller must copy the row
 * it selects before applying it. `randomOccupation` in `dcc_characters.ts` is the one place that
 * happens; see the note there.
 */
export const HUMAN_OCCUPATIONS: DCCOccupation[] = [
  {
    name: 'alchemist',
    trainedWeapon: {
      name: 'staff',
