...
export function byName(name: string): Tincture {
  ...
  throw new Error(`failed to find tincture ${name}`);
}

export function byType(type: string, tinctures: Tincture[]): Tincture[] {
  const filteredTinctures = [];

  for (let i = 0; i < tinctures.length; i++) {
    if (tinctures[i].type === type) {
      filteredTinctures.push(tinctures[i]);
    }
  }

  return filteredTinctures;
}
