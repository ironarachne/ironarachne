...
export function generateRegionOfControl(
    ...
    const region_types = getRegionTypesForTechnologyLevel(config.technology_level, config.region_types);
    const region_type = RND.weighted(region_types);
    const population_min = region_type.population_capacity * config.population_density_range[0];
    const population_max = region_type.population_capacity * config.population_density_range[1];
...
export function getRegionTypesForTechnologyLevel(number: number, region_types: Array<RegionType>): Array<RegionType> {
