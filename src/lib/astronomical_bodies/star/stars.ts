...
import { type AstronomicalBody, getAlbedoFromTemperature } from '$lib/astronomical_bodies/astronomical_bodies';
...
export function generateStar(config: StarGenerationConfig): AstronomicalBody {
  ...
  const mass: number = random.float(star_classification.min_mass, star_classification.max_mass);
  const radius: number = random.float(star_classification.min_radius, star_classification.max_radius) * 695700; // in km
  const gravity = getGravityFromMassAndRadius(mass, radius);
  const temperature = random.float(star_classification.min_temperature, star_classification.max_temperature);
  ...
  return {
    ...
    luminosity: random.float(star_classification.min_luminosity, star_classification.max_luminosity),
    mass: mass,
