...
/**
 * The planet classification table, in the order it was written. `getPlanetClassifications` sorts a
 * copy of this by name once and hands out that, so this array keeps its source order.
 *
 * Shared and read-only. Each row's `getRandomDescription` builds its own feature list per call, so
 * the shuffle inside `getDescriptionFromFeatures` never reaches this table.
 */
export const PLANET_CLASSIFICATIONS: PlanetClassification[] = [
  {
    name: 'arid',
    ...
    orbital_distance_min: 0.4,
    orbital_distance_max: 2.4,
  ...
  },
  {
    ...
    orbital_distance_min: 0.3,
    orbital_distance_max: 6.0,
    surface_pressure_min: 0.0001,
  ...
  },
  {
    ...
    orbital_distance_min: 5.0,
    orbital_distance_max: 40.0,
    surface_pressure_min: 0.0001,
