...
function scenesFor(
  render: (kind: 'canvas2d' | 'webgl') => string,
...
describe.each(SEEDS)('for seed "%s"', (seed) => {
  it('hands both backends the same planet scene', () => {
    const [canvas2d, webgl] = scenesFor((kind) =>
  ...
  it('hands both backends the same star scene', () => {
    const [canvas2d, webgl] = scenesFor((kind) =>
      renderStarPreviewImage(document, mockBody({ radius: 695700 }), 512, 512, seed, kind),
    );
  ...
  it('hands both backends the same star system scene', () => {
    const [canvas2d, webgl] = scenesFor((kind) =>
      renderStarSystemPreviewImage(document, system, 640, 160, seed, kind),
    );
...
describe('the scene both backends get', () => {
  ...
  it('is what each entry point builds, not a scene of some other kind', () => {
    const planet = scenesFor((kind) =>
      renderPlanetPreviewImage(document, mockBody({}), 512, 512, 'seed', kind),
    )[0];
    handed.canvas2d = [];
    handed.webgl = [];
    const star = scenesFor((kind) =>
      renderStarPreviewImage(document, mockBody({ radius: 695700 }), 512, 512, 'seed', kind),
    )[0];
    handed.canvas2d = [];
    handed.webgl = [];
    const wholeSystem = scenesFor((kind) =>
      renderStarSystemPreviewImage(document, system, 640, 160, 'seed', kind),
    )[0];
