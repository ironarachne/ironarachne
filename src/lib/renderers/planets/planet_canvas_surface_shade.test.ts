...
describe('shadePlanetDiskPixel', () => {
  ...
  it('center pixel is lit (not black)', () => {
    const c = shadePlanetDiskPixel(0, 0, 50, palette, shading, 'garden planet');
    const sum = c[0] + c[1] + c[2];
    expect(sum).toBeGreaterThan(0.15);
  });

  it('outside disk returns black', () => {
    const c = shadePlanetDiskPixel(60, 0, 50, palette, shading, 'garden planet');
    expect(c[0] + c[1] + c[2]).toBe(0);
  });

  it('limb is darker than subsolar-ish point', () => {
    const bright = shadePlanetDiskPixel(-20, 0, 50, palette, shading, 'garden planet');
    const limb = shadePlanetDiskPixel(0, 48, 50, palette, shading, 'garden planet');
    const b1 = bright[0] + bright[1] + bright[2];
    const b2 = limb[0] + limb[1] + limb[2];
    expect(b2).toBeLessThan(b1 * 0.95);
  });

  it('gas giant shading differs from terrestrial for same seed and palette', () => {
    const px = 5;
    const py = 12;
    const g = shadePlanetDiskPixel(px, py, 50, palette, shading, 'gas giant planet');
    const t = shadePlanetDiskPixel(px, py, 50, palette, shading, 'garden planet');
