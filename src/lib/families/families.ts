...
function addFounder(family: Family, config: FamilyGenerationConfig, rng: RNG) {
  ...
  const gender =
    config.dominantGender && config.dominantGender !== 'any'
...
function ageMembers(family: Family, years: number, config: FamilyGenerationConfig, rng: RNG) {
  for (const member of family.members) {
    ...
    if (member.ageCategory.name !== 'infant' && member.personalityTraits.length === 0) {
      member.personalityTraits = PersonalityTraits.getRandomPersonalityTraits(
        rng.randomString(16),
        rng.int(1, 3),
...
function generateMarriages(family: Family, config: FamilyGenerationConfig, rng: RNG) {
  const eligible = family.members.filter(
    (m) =>
  ...
  for (const member of eligible) {
    ...
    const spouseSpecies =
      config.allowCrossSpeciesMarriages &&
      rng.float(0, 1) < (config.crossSpeciesMarriageChance || 0)
    ...
    if (
      config.allowSameGenderMarriage &&
      rng.float(0, 1) < (config.sameGenderMarriageChance || 0)
    ) {
    ...
    if (config.dominantGender && config.dominantGender !== 'any') {
      if (
        spouse.gender.name === config.dominantGender &&
        member.gender.name !== config.dominantGender
      ) {
...
function generateIllegitimateChildren(family: Family, config: FamilyGenerationConfig, rng: RNG) {
  ...
  const eligible = family.members.filter((m) => !m.tags.includes('dead') && isFertile(m));
...
function linkSiblings(family: Family, childId: string, parentId: string, rng: RNG) {
  const parentRels = family.relationships.filter(
    (r) => r.originatorId === parentId && r.type.name === 'parent' && r.recipientId !== childId,
  );
  for (const rel of parentRels) {
    ...
    const exists = family.relationships.some(
      (r) =>
        r.type.name === 'sibling' &&
...
function getCouples(family: Family): [string, string][] {
  ...
  for (const rel of family.relationships) {
    if (
      rel.type.name === 'spouse' &&
      !processed.has(rel.originatorId) &&
      !processed.has(rel.recipientId)
    ) {
