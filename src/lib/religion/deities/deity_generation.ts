...
export function generate(seed: string, config: DeityGenerationConfig): Deity {
  ...
  deity.holyItem =
    config.hasHolyItem && primaryDomain && primaryDomain.holyItems.length > 0
      ? rng.item(primaryDomain.holyItems)
      : null;
  deity.holySymbol =
    config.hasHolySymbol && primaryDomain && primaryDomain.holySymbols.length > 0
...
export function describeDeity(deity: Deity, rng: RNG): string {
  ...
  desc += '. ';
  if (deity.holyItem) {
    desc += Words.capitalize(
      `${Words.pronoun(deity.gender.name, 'possessive')} holy item is ${Words.article(deity.holyItem)} ${deity.holyItem}. `,
    );
  }
  if (deity.holySymbol) {
    desc += Words.capitalize(
      `${Words.pronoun(deity.gender.name, 'possessive')} holy symbol is ${Words.article(deity.holySymbol)} ${deity.holySymbol}. `,
    );
  }
