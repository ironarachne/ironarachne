...
export function generateDomainSet(seed: string, domains: Domain[], filter: DomainFilter, minDomains: number, maxDomains: number): DomainSet {
    ...
    let relatedDomains = filteredDomains.filter(domain => domain.tags.some(tag => primary.tags.includes(tag)));
