...
export function generate(seed: string, config: PantheonGenerationConfig): Pantheon {
	...
	for (let i = 0; i < numDeities; i++) {
        ...
        const species = config.speciesOptions && config.speciesOptions.length > 0 ? rng.item(config.speciesOptions) : human;
		...
		const deityConfig = {
			...
			domainFilter: { name: null, hasHolyItems: null, hasHolySymbols: null, hasEnchantments: null, requiredTags: [], excludedTags: [] },
