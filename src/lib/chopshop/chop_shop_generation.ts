/**
 * The chop shop's prose, assembled from five phrase tables: exterior, entry, product displays,
 * customers, and the back room.
 *
 * `generate` returns the bare paragraph, as it always has; `generateChopShop` wraps it in the
 * library's one type so that a snapshot, an editor and an export have a value to hold rather than
 * a string to pass around.
 */
...
/** A chop shop as a value. The paragraph is the whole of it. */
export function generateChopShop(rng: RNG): ChopShop {
