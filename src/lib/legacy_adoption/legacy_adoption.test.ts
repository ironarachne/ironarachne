...
describe('adoptLegacySaves', () => {
  it('adopts a real saved coat of arms, culture, and religion into one new project', async () => {
  ...
  it('records no provenance, because the legacy scopes stored no origin to record', async () => {
  ...
  it('migrates a version 1 coat of arms through the kind registry on the way in', async () => {
    ...
    const read = await readArtifact(ARTIFACT_KINDS, result.projectId!, summary.id);
  ...
  it('leaves every legacy scope exactly as it found it', async () => {
    writeAllLegacyScopes();
  ...
  it('runs once when two callers ask at the same time', async () => {
    writeAllLegacyScopes();

    // The root layout and the project bar both call this on a page load. Before the store was
    // asynchronous the first call finished before the second began; now they overlap, and two runs
    // would each see an empty adoption record and adopt everything a second time.
    const [first, second] = await Promise.all([adopt(1_000), adopt(1_000)]);
  ...
  it('adopts nothing a second time', async () => {
    writeAllLegacyScopes();
    const first = await adopt(1_000);

    const second = await adopt(2_000);
  ...
  it('picks up an item saved after the previous run without touching the rest', async () => {
  ...
  it('does not touch a browser that never saved anything', async () => {
    const result = await adopt();
  ...
  it('adopts the one scope that has anything in it', async () => {
  ...
  it('reports a scope whose envelope it cannot parse, and adopts the others', async () => {
  ...
  it('skips a malformed item with its reason, and keeps the good ones beside it', async () => {
  ...
  it('creates no project when it could not read a single item', async () => {
  ...
  it('records no key against a skipped item, so a later build can still adopt it', async () => {
    const good = legacyCultureSnapshot('good');
  ...
  it('adopts two saved items that share a name rather than counting the second as done', async () => {
  ...
  it('adopts into the project the last run used', async () => {
    const kept = namedCultureSnapshot('first', 'Aurelian');
    writeLegacyScope(CULTURE_SCOPE, 'cultures', [kept]);
    const first = await adopt(1_000);
    await createProject({ name: 'Somewhere else' }, { now: 1_500 });
  ...
  it('creates a fresh project when the one it used has been deleted', async () => {
    const kept = namedCultureSnapshot('first', 'Aurelian');
    writeLegacyScope(CULTURE_SCOPE, 'cultures', [kept]);
    const first = await adopt(1_000);
    await deleteProject(first.projectId!);
  ...
  it('gives the project a name and a description the user can change', async () => {
  ...
  it('honours a caller-supplied project name', async () => {
    writeLegacyScope(CULTURE_SCOPE, 'cultures', [legacyCultureSnapshot('first')]);

    const result = await adoptLegacySaves(ARTIFACT_KINDS, { now: 1_000, projectName: 'Ashfall' });

    expect(getProject(result.projectId!)?.name).toBe('Ashfall');
  });

  it('stamps every artifact in a run with the same timestamps', async () => {
    writeAllLegacyScopes();

    const result = await adopt(4_242);
...
describe('the adoption notice', () => {
  it('is left for the user by a run that adopted something', async () => {
    writeAllLegacyScopes();

    const result = await adopt(1_000);
  ...
  it('is not raised by a run that adopted nothing', async () => {
    await adopt();

    expect(legacyAdoptionNotice()).toBeNull();
  });

  it('adds up runs the user has not read yet', async () => {
    const kept = namedCultureSnapshot('first', 'Aurelian');
    writeLegacyScope(CULTURE_SCOPE, 'cultures', [kept]);
    await adopt(1_000);
    writeLegacyScope(CULTURE_SCOPE, 'cultures', [kept, namedCultureSnapshot('second', 'Borvath')]);
  ...
  it('is cleared when acknowledged, without forgetting what was adopted', async () => {
    writeAllLegacyScopes();
    const result = await adopt(1_000);
    ...
    expect((await adopt(2_000)).adopted).toEqual([]);
    expect(listArtifacts(result.projectId!)).toHaveLength(3);
  });

  it('acknowledging nothing writes nothing', async () => {
    acknowledgeLegacyAdoptionNotice();
