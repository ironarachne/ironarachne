...
export function getSetExcluding(tincturesToRemove: Tincture[], tincturesToCompare: Tincture[]): Tincture[] {
  let result = [];
...
export function randomChargeTincture(): Tincture {
  ...
  const colorChance = RND.chance(100);
...
export function randomSetOfTypes(types: string[], count: number): Tincture[] {
  let result = [];

  if (types.length < 1) {
    console.info('tried to get random set of tinctures but specified zero types');
    return [];
  }

  let options = ofTypes(types);

  for (let i=0;i<count;i++) {
    result.push(RND.item(options));
  }
