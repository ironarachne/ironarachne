...
export function generate() {
  ...
  for (let i = 0; i < character.characterClass.abilities.length; i++) {
    if (character.characterClass.abilities[i].type === "bonus focus") {
      let newFocusType = RND.item(character.characterClass.abilities[i].focusTypes);
      ...
      let skillType = RND.item(character.characterClass.abilities[i].skillTypes);
  ...
  for (let i = 0; i < character.focuses.length; i++) {
    ...
    if (character.focuses[i].levelOneEffect.type === "specific bonus skill") {
      ...
      let skillAddressed = false;
      let skillType = RND.item(character.focuses[i].levelOneEffect.skillTypes);
      let newSkill = randomSkillOfType(skillType);
      ...
      let skillAddressed = false;
      let newSkill = RND.item(character.focuses[i].levelOneEffect.skillNames);
  ...
  let skillAddressed = false;
  let hobbySkill = RND.item([randomSkillOfType("non-combat"), randomSkillOfType("combat")]);
...
function randomBackground() {
  ...
  let background = RND.item(backgrounds);
...
function randomStartingSkills(background) {
  ...
  for (let i = 0; i < startingSkills.length; i++) {
    ...
    if (startingSkills[i] === "Any Combat") {
      ...
      skill.name = RND.item(["Shoot", "Trade"]);
    } else {
