...
export function generate() {
  ...
  for (let i = 0; i < character.skills.length; i++) {
    ...
    if (character.skills[i].name === "Biopsionics") {
      ...
      if (character.skills[i].level === 0) {
        character.abilities.push(
          new SpecialAbility("Psychic Refinement-0: The adept can visually and audibly detect the use of psychic powers. If both the source and target are visible to the metapsion, they can tell who’s using the power, even if it’s normally imperceptible. They gain a +2 bonus on any saving throw versus a psionic power.")
        );
      } else if (character.skills[i].level === 1) {
        character.abilities.push(
          new SpecialAbility("Psychic Refinement-1: The adept can visually and audibly detect the use of psychic powers. If both the source and target are visible to the metapsion, they can tell who’s using the power, even if it’s normally imperceptible. They gain a +2 bonus on any saving throw versus a psionic power. The metapsion’s maximum Effort increases by an additional point.")
        );
        character.effort++;
      ...
      if (character.skills[i].level === 0) {
        character.abilities.push(
          new SpecialAbility("Telekinetic Manipulation-0: The psychic can exert force as if with one hand and their own strength.")
        );
      } else if (character.skills[i].level === 1) {
        character.abilities.push(
      ...
      if (character.skills[i].level === 0) {
        character.abilities.push(
          new SpecialAbility("Telepathic Contact-0: Observe emotional states in a target. Intense emotions provide a single word or image related to the focus of the feelings.")
        );
      } else if (character.skills[i].level === 1) {
        character.abilities.push(
...
export class Focus {
  ...
  constructor(name: string, focusType: string, levelOneDescription: string, levelOneEffect: BonusSkill | BonusSkillFromList | BonusSkillOfType | BonusHP | InnateAC, levelTwoDescription: string) {
    this.name = name;
...
export class CharacterClass {
  ...
  constructor(name: string, attackBonus: number, hitPointRoll: string, abilities: (BonusFocus | BonusSkillOfType | SpecialAbility | EffortAbility)[]) {
    this.name = name;
  ...
  return [
    new CharacterClass(
      ...
      [
        ...
        new SpecialAbility("Once per scene, you can reroll a failed skill check, taking the new roll if it’s better."),
        new SpecialAbility("When you advance an experience level, you gain a bonus skill point that can be spent on any non-combat, non-psychic skill. You can save this point to spend later if you wish."),
      ]
    ),
    new CharacterClass(
      "Psychic",
    ...
    new CharacterClass(
      ...
      [
        new BonusFocus(["non-combat"]),
        new SpecialAbility("Gain an extra skill point every time you gain a character level which can be spent on any non-psychic, non-combat skill."),
        new BonusSkillOfType(["psychic"]),
        new EffortAbility(),
      ]
    ),
...
export class Background {
  ...
  constructor(name: string, equipmentPackage: string, freeSkill: string, quickSkills: string[], learningSkills: string[]) {
    this.name = name;
