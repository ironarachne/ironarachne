...
export function generate(itemCategory: string, valueThreshold: number): Merchant {
  ...
  let description = RND.item([
    ...
    `${character.firstName} ${character.lastName}, ${Words.article(character.species.name)} ${
      character.species.name
    } ${character.ageCategory.noun}, is ${Words.article(itemCategory)} ${itemCategory} merchant.`,
  ]);
  let wares = StockList.getList(itemCategory, 10, valueThreshold);
  let priceVariance = random.float(0.8, 1.2);
  if (priceVariance > 1.0) {
    description +=
      ' ' + Words.capitalize(character.gender.subjectivePronoun) + ' charges more than others.';
  } else if (priceVariance < 1.0) {
    description +=
      ' ' + Words.capitalize(character.gender.subjectivePronoun) + ' charges less than others.';
  }
