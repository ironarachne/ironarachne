...
class Domain {
  ...
  weaponEffects;
  otherEffects;
  constructor(name, holyItems, holySymbols, weaponEffects, otherEffects) {
    this.name = name;
...
function all() {
  return [
    ...
    new Domain("dawn", ["torch"], ["radiant sun rising"], [], ["glows bright yellow on command"]),
    new Domain(
      "death",
    ...
    new Domain("healing", ["wand"], ["wreath of flowers", "open hand"], [], ["can heal any wound"]),
    new Domain("hope", ["hoop"], ["open hand", "circle"], [], ["glows with a soft, warm light"]),
    new Domain(
      "horses",
    ...
    new Domain("sky", ["flute", "kite"], ["cloud"], [], ["allows its bearer to fly"]),
    new Domain(
      "spring",
      ["staff"],
...
function getSpecificDomain(name) {
  ...
  console.error("failed to find domain name: " + name);
  return allDomains[0];
}

export { all as a, getSpecificDomain as b, getAllDomainNames as g };
//# sourceMappingURL=domains-b9478194.js.map
