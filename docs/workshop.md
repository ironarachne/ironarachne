...
| Artifact editing           | **Framework built (#39); culture (#40), religion (#41), and settlement (#20) fill it.** `openArtifactForEditing`, the dirty/save lifecycle, the destructive re-roll, and the unsaved-edits guard, with a per-kind editor slot — and a per-kind **viewer** slot beside it, for a kind that can draw itself (6.3) before it can be edited (4.1). Heraldry was the one that did, until #51 gave it an editor. |
...
Heraldry's Beta was assessed rather than inherited from "approaching Beta": it cleared sections
1–3, 6, and 7.1–7.2, and what held it short of Release-ready was 4.1 — `ARTIFACT_EDITORS` gave it a
viewer and no editor, so a saved coat of arms could be seen and downloaded and not changed. #51
gave it an editor and it is Release-ready now; the sentence is kept because it is the worked example
of a level being assessed against the bar rather than assumed.
